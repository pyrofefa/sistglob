...
@Component({
  ...
  templateUrl: './simmoscas.component.html',
  styleUrls: ['./simmoscas.component.scss'],
  standalone: false,
})
export class SimmoscasComponent implements OnInit {
  @Input() name: any;
  @Input() id: any;
  @Input() campana: any;
  ...
  constructor(
    ...
    public capturas: SimmoscasService,
          ...
          this.capturas.capturaId(this.id, this.fecha).then((res) => {
            if (res[0].id != null) {
              for (let result of res) {
                ...
                this.captura.gancho = result.gancho;
                this.captura.recebado = result.recebado;
  ...
  getFenologias() {
    this.fenologia
      ...
      .then((res) => {
        this.fenologias = res;
        this.compareWithFenologia = this.compareWithFn;
      })
      .catch((error) => {
        alert(error);
      });
  ...
  async guardarRegistro(isUpdate: boolean) {
    ...
    try {
      const capturaData = buildCapturaSimmoscas(
        this.captura,
        this.junta_id,
        this.personal_id,
        this.user_id,
